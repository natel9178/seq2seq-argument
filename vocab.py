...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.start_of_word] + [self.char2id.get(c, self.char2id['<unk>']) for c in w] + [self.end_of_word] for w in s] for s in sents]
        # END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_padded = pad_sents_char(
            self.words2charindices(sents), self.char2id['<pad>'])
        # Output of pad_sents_char 0 is (4, 6, 21), (sentence_num/batch, max_sent_len, max_word_len)
        sents_var = torch.tensor(
            sents_padded, dtype=torch.long, device=device).permute(1, 0, 2).contiguous()
        return sents_var
        # END YOUR CODE
