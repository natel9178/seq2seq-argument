...
def pad_sents_char(sents, char_pad_token):
    ...
    sents_padded = [[w + [char_pad_token] * (max_word_length - len(w)) if (max_word_length - len(
        w)) >= 0 else w[:max_word_length] for w in s] + ([[char_pad_token] * max_word_length] * (max_sent_length - len(s))) for s in sents]
    # END YOUR CODE
